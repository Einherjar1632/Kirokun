...
interface TranscriptionSettingsScreenProps {
  onBack: () => void;
}
...
export const TranscriptionSettingsScreen: React.FC<TranscriptionSettingsScreenProps> = ({ onBack }) => {
  ...
  const saveSettings = async () => {
    try {
      ...
      Alert.alert('設定保存', '文字起こし設定が正常に保存されました');
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <View style={styles.titleContainer}>
          <Text style={styles.title}>📝 文字起こし設定</Text>
          <Text style={styles.subtitle}>音声を文字起こしする際に使用するAIへの指示文を設定できます</Text>
        </View>
